...
for (c in chrs) { cat(c); cat("-")
  ...
  for (v in met_context){ cat(v, fill = TRUE)
    ...
    for (p in parents){ cat(p); cat(": ")
      ...
      part_size<-1000000
      n_parts<-ceiling(length(as.numeric(positions_middle))/part_size)
      parts<-list()
      cat("PARTS:")
      ...
      if (any(is.na(positions))){positions<-positions[-which(is.na(positions))]}
      ...
      windows_part<-as.character(as.numeric(format(Cs_per_window[,1], scientific = FALSE)))
      Cs_per_window<-as.numeric(Cs_per_window[,2]); names(Cs_per_window)<-windows_part
      windows_part<-windows_part[-which((windows_part%in%windows)==FALSE)]
      for (w in windows_part){methy_table[w,p]<-sum(as.numeric(methy_table[w,p]),Cs_per_window[w], na.rm = T)}#w
      }#g

...
pop_info<-read.csv("/home/fcasale/Desktop/Paper_1/crossing design.csv")
...
colnames(tabla)<-c("", treatments)
  for (p in Populations){ cat(p); cat(": ")   
    ...
    for (v in met_context){ cat(v); cat("-")
      ...
      coldspots_offspring<-cbind(window_type_list[[v]]$coldspots[[p]], e)
      colnames(coldspots_offspring)<-colnames(data_table)
      data_table<-rbind(data_table,coldspots_offspring)
...
tabla_final<-cbind(tabla_final, tabla)
}#e
...
tabla_group_0<-matrix(ncol = length(parents)+1+1, nrow = 0); colnames(tabla_group_0)<-c("",parents, e)

for (e in window_types){ cat(e, fill = TRUE)
tabla_group<-matrix(ncol = length(parents)+1+1, nrow = 0); colnames(tabla_group)<-c("",parents, e)
  tabla_group<-rbind(tabla_group, cbind(e, "", "", "", ""))  
  for (p in Populations){ cat(p); cat("-") 
    #select parents   
    P1<-pop_info[which(pop_info[,1]==p),2]
    P2<-pop_info[which(pop_info[,1]==p),3]
    treatments<-c(P1, P2, e)
    ...
    for (v in met_context){ cat(v); cat("-")
      ...
      #add distal windows in pop
      data_table<-rbind(data_table,cbind(window_type_list[[v]][[e]][[p]], e))
      data_table<-as.data.frame(data_table)
      data_table$methylation<-as.numeric(as.character(data_table$methylation))
      ...
      #if (is.null(ncol(letter_table))){
        for (c in treatments){tabla_g[v,c]<-letter_table$Letters[c]}  
      # } else {
    ...
    #write.xlsx(tabla_p, "/home/fcasale/Desktop/Paper_2/3_RILs/7_window_types/Results/B_methylation/B.3.2_all_methy_types_windows_unified_WILCOX.xlsx", sheetName = paste(e,p, sep = "_"), append = TRUE)
  }#p
  #save table groups
  tabla_group_0<-rbind(tabla_group_0, tabla_group)
}#e 
write.xlsx(tabla_group_0, "/home/fcasale/Desktop/Paper_2/3_RILs/7_window_types/Results/B_methylation/B.3.2_NEW_all_methy_types_windows_unified_WILCOX_GROUPING.xlsx", row.names = FALSE)

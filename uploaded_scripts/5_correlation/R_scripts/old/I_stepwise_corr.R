...
for (c in 1:7){ cat(c);cat("-")
  ...
  for (w in windows){
    ...
    #LUEGO INVESTIGAR PORQUE NO HAY GEN.DIST ENTRE ALGUNOS PADRES, BY NOW I ASSUME NO DISTANCE
...
saveRDS(sw_list, "/scratch/Federico/3_RILs/5_correlation/Results/I.1_step_wise_regression_per_window.RDS")

exp_variables<-names(variables)

sw_list<-readRDS("/scratch/Federico/3_RILs/5_correlation/Results/I.1_step_wise_regression_per_window.RDS")
...
for (c in 1:7){ cat(c);cat("-")
  ...
  #take direction of regression as binomial variable
  direction<-chr_table[,3]>0; chr_table<-cbind(c, chr_table, direction)
  chr_table[,4]<-round(chr_table[,4], digits = 2)
  chr_table[,5]<-round(chr_table[,5], digits = 5)
  colnames(chr_table)<-c("chr", "window", "variable", "reg_coeff", "P_value", "positive")
  significant_variables_list[[c]][["graphic_table"]]<-chr_table
}#c

saveRDS(significant_variables_list, "/scratch/Federico/3_RILs/5_correlation/Results/I.2_sw_reg_per_win_significant_variables.RDS")

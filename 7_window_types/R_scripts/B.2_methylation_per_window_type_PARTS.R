...
#generate chromosome parts
distal_telomeric<-list()
distal_proximal<-list()
for (p in Populations){
  distal_telomeric[[p]]<-list()
  distal_proximal[[p]]<-list()
  for (c in 1:7){
  distal_telomeric[[p]][[c]]<-list()
  distal_proximal[[p]][[c]]<-list()
  #first arm
  distal_1<-distal_windows[[p]][[c]][which(as.numeric(distal_windows[[p]][[c]][,2])<as.numeric(pericentromere_pops[[p]][c,4])),]
  distal_1_medium<-round(nrow(distal_1)/2)
  distal_1_telomeric<-distal_1[1:(distal_1_medium-1),]
  distal_1_proximal<-distal_1[distal_1_medium:nrow(distal_1),]  
  #second arm
  distal_2<-distal_windows[[p]][[c]][which(as.numeric(distal_windows[[p]][[c]][,2])>as.numeric(pericentromere_pops[[p]][c,5])),]
  distal_2_medium<-round(nrow(distal_2)/2)
  distal_2_proximal<-distal_2[1:(distal_2_medium-1),]
  distal_2_telomeric<-distal_2[distal_2_medium:nrow(distal_2),]  
  #sum similar parts
  distal_telomeric[[p]][[c]]<-rbind(distal_1_telomeric, distal_2_telomeric)
  distal_proximal[[p]][[c]]<-rbind(distal_1_proximal, distal_2_proximal)
  }#c
}#p
saveRDS(list(distal_telomeric, distal_proximal), "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.0_distal_parts.RDS")

#subset coldspots 
coldspots<-readRDS("/scratch/Federico/3_RILs/7_window_types/Results/A.2_coldspots/A.2.1_coldspots_per_pop_WINDOWS.RDS")
coldspots_telomeric<-list()
coldspots_proximal<-list()
for (p in Populations){
  coldspots_telomeric[[p]]<-list()
  coldspots_proximal[[p]]<-list()
  for (c in 1:7){
    coldspots_telomeric[[p]][[c]]<-coldspots[[p]][[c]][which(coldspots[[p]][[c]][,2]%in%distal_telomeric[[p]][[c]][,2]),]
    coldspots_proximal[[p]][[c]]<-coldspots[[p]][[c]][which(coldspots[[p]][[c]][,2]%in%distal_proximal[[p]][[c]][,2]),]
  }#c
}#p
...
hotspots_telomeric<-list()
hotspots_proximal<-list()
hotspots_pericentromeric<-list()
hotspots_distal<-list()
for (p in Populations){
  hotspots_telomeric[[p]]<-list()
  hotspots_proximal[[p]]<-list()
  hotspots_pericentromeric[[p]]<-list()
  hotspots_distal[[p]]<-list()
  for (c in 1:7){
    hotspots_telomeric[[p]][[c]]<-hotspots[[p]][[c]][which(hotspots[[p]][[c]][,2]%in%distal_telomeric[[p]][[c]][,2]),]
    hotspots_proximal[[p]][[c]]<-hotspots[[p]][[c]][which(hotspots[[p]][[c]][,2]%in%distal_proximal[[p]][[c]][,2]),]
    hotspots_pericentromeric[[p]][[c]]<-hotspots[[p]][[c]][which(hotspots[[p]][[c]][,2]%in%peri_windows[[p]][[c]][,2]),]
    hotspots_distal[[p]][[c]]<-rbind(hotspots_telomeric[[p]][[c]], hotspots_proximal[[p]][[c]])  
  }#c
}#p


#make list
window_type_list<-list(peri_windows, distal_windows, distal_proximal, distal_telomeric, coldspots, coldspots_proximal, coldspots_telomeric, hotspots_distal, hotspots_pericentromeric, hotspots_proximal, hotspots_telomeric)
window_types<-c("peri_windows", "distal_windows", "distal_proximal", "distal_telomeric", "coldspots", "coldspots_proximal", "coldspots_telomeric", "hotspots_distal", "hotspots_pericentromeric", "hotspots_proximal", "hotspots_telomeric")
names(window_type_list)<-window_types
saveRDS(window_type_list, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.1_window_types.RDS")

#delete useless categories
window_types<-window_types[-which(window_types%in%c("distal_windows","coldspots", "hotspots_distal"))]
window_type_list<-window_type_list[-which(names(window_type_list)%in%c("distal_windows","coldspots", "hotspots_distal"))]
...
saveRDS(methy_list_windows, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.2_methylation_prop_per_window_type.RDS")

# #delete coldspots and hotspots from the regions windows #I WONT DO THIS
# for (m in met_contexts){cat(m); cat(":")
#   for (e in c("peri_windows", "distal_proximal", "distal_telomeric")){ cat(e); cat("-")
#     for (p in Populations){
#       for (c in 1:7){
#         coldspots_proximal<-which(methy_list_windows[[m]][[e]][[p]][[c]][,1]%in%methy_list_windows[[m]][["coldspots_proximal"]][[p]][[c]][,1])
#         coldspots_telomeric<-which(methy_list_windows[[m]][[e]][[p]][[c]][,1]%in%methy_list_windows[[m]][["coldspots_telomeric"]][[p]][[c]][,1])
#         hotspots_telomeric<-which(methy_list_windows[[m]][[e]][[p]][[c]][,1]%in%methy_list_windows[[m]][["hotspots_telomeric"]][[p]][[c]][,1])
#         hotspots_proximal<-which(methy_list_windows[[m]][[e]][[p]][[c]][,1]%in%methy_list_windows[[m]][["hotspots_proximal"]][[p]][[c]][,1])
#         hotspots_pericentromeric<-which(methy_list_windows[[m]][[e]][[p]][[c]][,1]%in%methy_list_windows[[m]][["hotspots_pericentromeric"]][[p]][[c]][,1])
#         to.delete<-c(coldspots_proximal, coldspots_telomeric, hotspots_telomeric, hotspots_proximal, hotspots_pericentromeric)
...
file.remove("/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.3_all_methy_types_windows_DIF_AMONG_CHR_TUKEY.xlsx")
...
write.xlsx(tabla, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.3_all_methy_types_windows_DIF_AMONG_CHR_TUKEY.xlsx", row.names = FALSE)
###################################################################################################
...
saveRDS(methy_list_windows, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.4_methylation_prop_per_window_type_unified.RDS")
#methy_list_windows<-readRDS("/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.4_methylation_prop_per_window_type_unified.RDS")

###################### check if means are different with ANOVA + Tukey #############################
file.remove("/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.5.1_all_methy_types_windows_unified_TUKEY.xlsx")
...
write.xlsx(tabla, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.5.1_all_methy_types_windows_unified_TUKEY.xlsx")
####################################################################################################

########### check if means are different with multiple kruskall-wallis (WILCOX) ####################
file.remove("/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.5.2.1_all_methy_types_windows_unified_WILCOX.xlsx")
file.remove("/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.5.2.2_all_methy_types_windows_unified_WILCOX_GROUPING.xlsx")

tabla_group<-matrix(ncol = length(window_types)+1, nrow = 0); colnames(tabla_g)<-c("",window_types)
for (p in Populations){ cat(p); cat("-") 
  ...
  write.xlsx(tabla_p, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.5.2.1_all_methy_types_windows_unified_WILCOX.xlsx", sheetName = p, append = TRUE)
  #arrange table group
...
tabla_group<-rbind(c("","Pericentomeric", "Distal proximal", "Distal telomeric", "Distal proximal", "Distal telomeric", "Pericentromeric", "Distal proximal", "Distal telomeric"), tabla_group)
colnames(tabla_group)<-c("","Chromosome regions", "", "", "Coldspots", "", "Hotspots", "", "")
#save
write.xlsx(tabla_group, "/scratch/Federico/3_RILs/7_window_types/Results/B_methylation/B.2.5.2.2_all_methy_types_windows_unified_WILCOX_GROUPING.xlsx", row.names = FALSE)

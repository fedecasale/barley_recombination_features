...
SVs_proportion<-readRDS(paste("/home/fcasale/Desktop/Paper_2/3_RILs/3_SVs/Results/H_SVs_proportion_per_window/H.1.2_SVs_proportion_per_window=10000.RDS", sep = ""))

#I will normalize data cause, some windows have fractions higher than 1
#Ahora normalizo por cada pop para cada chr
# for (p in Populations){ cat(p); cat(": ")
# for (c in 1:7){ cat(c); cat("-")
# data<-unlist(SVs_proportion[[p]][[c]])
# MAX<-max(data, na.rm = TRUE); MIN<-min(data, na.rm = TRUE); MEAN<-mean(data, na.rm = TRUE)
# normalize<-function(x){x<-(x-MEAN)/(MAX-MIN); return(x)}  
# normalized_values<-lapply(SVs_proportion[[p]][[c]], normalize)
# normalized_values<-unlist(normalized_values)
# C_MIN<-min(normalized_values)
# normalized_values<-normalized_values+abs(C_MIN) #como me quedan negativos, llevo el min a 0
# if (any(normalized_values>1)){cat("OH NO, SOME GET HIGHER THAN 1")}
# n_val_list<-as.list(normalized_values); names(n_val_list)<-names(SVs_proportion[[p]][[c]])
# SVs_proportion[[p]][[c]]<-n_val_list
# }#c
# cat(fill = TRUE)
# }#p
...
svs_list_windows<-readRDS("/home/fcasale/Desktop/Paper_2/3_RILs/7_window_types/Results/C_SVs/C.2.1_svs_prop_per_window_type.RDS")
...
for (p in Populations){
  for (c in 1:7){
   ...
   start<-regions[r,1]
   if (start%in%svs_list_windows$coldspots_proximal[[p]][[c]][,1]){proximal[[paste(regions[r,1])]]<-regions[r,]}
   if (start%in%svs_list_windows$coldspots_telomeric[[p]][[c]][,1]){telomeric[[paste(regions[r,1])]]<-regions[r,]}
   }#r
...
for (p in Populations){cat(p);cat(":")
  ...
  windows_around_break_list[[w]][[p]]<-list()
  
  for (c in 1:7){ cat(c);cat("-")

     #get windows of CO breakpoint
     break_windows<-as.numeric(svs_list_windows[[w]][[p]][[c]][,1])
    ...
    if (any(break_windows<=100000)){break_windows<-break_windows[-which(break_windows<=100000)]}
      ...
      similar_windows<-svs_list_windows[[w]][[p]][[c]][,1]
    ...
    windows_around_break_list[[w]][[p]][[c]]<-windows_around_break_table
  ...
  graphic_list[[w]][[p]]<-svs_prop_around_break
...
saveRDS(windows_around_break_list, "/home/fcasale/Desktop/Paper_2/3_RILs/7_window_types/Results/E_windows_around/E.1.0_windows_around_break_list.RDS")

saveRDS(graphic_list, "/home/fcasale/Desktop/Paper_2/3_RILs/7_window_types/Results/E.1.1_graphic_list.RDS")
...
#graphic_table<-matrix(ncol = 5, nrow=0); colnames(graphic_table)<-c("Population", "window_type", "SVs_proportion","chr", "window")
graphic_table<-matrix(ncol = 4, nrow=0); colnames(graphic_table)<-c("Population", "window_type", "SVs_proportion", "window")

for (w in window_types){
for (p in Populations){
#for (c in 1:7){
  for (i in 1:ncol(graphic_list[[w]][[p]])){
    col2win<-(i-6)*10
    #graphic_table<-rbind(graphic_table,cbind(p, w, graphic_list[[w]][[p]][c,i], c, col2win))
    graphic_table<-rbind(graphic_table,cbind(p, w, mean(graphic_list[[w]][[p]][,i]), col2win))
  }#i
#}#c
}#p
}#w
...
graphic_table$SVs_proportion[which(graphic_table$SVs_proportion<0)]<-0
...
pdf("/home/fcasale/Desktop/Paper_2/3_RILs/7_window_types/Results/E_windows_around/E.1.2_windows_around_SVs.pdf", width = 8, height = 10)

ggplot()+
  geom_line(data = graphic_table, aes(x = window, y = SVs_proportion, color = window_type)) +
  geom_vline(xintercept = 0, linetype = "dashed")+
  
  scale_y_continuous(name = "Window SVs' relative occupancy", 
                     #limits = c(0, 0.05),
  ...
  facet_grid(rows = vars(Population), scales = "free", space = "free_y") +
...
# levels(graphic_table$SVs_proportion)
# warnings()
###################################
...
#    for (r in 1:nrow(regions)){
#    start<-regions[r,1] 
#    if (start%in%svs_list_windows$coldspots_proximal[[p]][[c]][,1]){proximal[[paste(r)]]<-regions[r,]}
#    if (start%in%svs_list_windows$coldspots_telomeric[[p]][[c]][,1]){telomeric[[paste(r)]]<-regions[r,]}
#    }#r
#    coldspots_regions[[p]][[c]]<-list(proximal, telomeric); names(coldspots_regions[[p]][[c]])<-c("proximal", "telomeric")
#   }#c
# }#p
...
#if (w%in%c("coldspots_proximal", "coldspots_telomeric")){
  #  if (w == "coldspots_proximal"){regions<-coldspots_regions[[p]][[c]]$proximal} else {regions<-coldspots_regions[[p]][[c]]$telomeric}
  #   start_windows<-c(); for (r in 1:length(regions)){start_windows<-c(start_windows, regions[[r]][1])}
  #   end_windows<-c(); for (r in 1:length(regions)){end_windows<-c(end_windows, regions[[r]][2])}; end_windows<-end_windows+1
  #   chr_windows<-as.numeric(names(SVs_proportion[[p]][[c]]))
  #   #get windows around
  #   if (any(start_windows<=100000)){start_windows<-start_windows[-which(start_windows<=100000)]}
  #   windows_around_break_table<-matrix(ncol = 21, nrow = 0)
  #   for (b in 1:length(start_windows)){
  #     windows_before_coldspot<-chr_windows[(which(chr_windows==start_windows[b])-10):which(chr_windows==start_windows[b])]
  #     windows_after_coldspot<-chr_windows[which(chr_windows==end_windows[b]):(which(chr_windows==end_windows[b])+10)][-1]
  #     windows_around_break<-c(windows_before_coldspot, windows_after_coldspot)  
  #     windows_around_break_table<-rbind(windows_around_break_table, windows_around_break)
  #   }#b
  #   
  # #convert windows to SVs values
  ...
  # 
  # # #add window name
  # # colnames(windows_around_break_table)[-10:10]
  # 
  # #break window will be converted to region average
  # for (r in 1:length(regions)){
  # start<-regions[[r]][1]
  # end<-regions[[r]][2]
  # region_values<-values[which(names(values)==as.character(start)):which(names(values)==as.character(end+1))]
  # windows_around_break_table[r,11]<-mean(region_values)
  # }#r
  # 